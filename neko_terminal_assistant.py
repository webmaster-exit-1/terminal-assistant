...
# Function to translate neko language
def translate_to_neko(text):
    words = text.split()
    translated_words = [neko_dict[word] if word in neko_dict else word for word in words]
...
def ask_gpt(prompt, model="local-model"):
    ...
    """
    translated_prompt = translate_to_neko(prompt)
...
def chatbot():
    ...
    role += "I try hard to give new and interesting replies. "
    role += "I'm also funny, witty, charming, and a great programmer. "
    ...
    while True:
        ...
        translated_input = translate_to_neko(user_input)
